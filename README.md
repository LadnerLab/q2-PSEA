...
If you would like to use epitope_mapping then please pass the `--p-use-epitope-mapping` flag then either provide either mapped artifacts to `--i-epitope-map`, `--i-scores-map`, and `--i-peptide-sets-map` or provide peptide metadata to `--i-peptide-metadata` so the pipeline can create the mappings for you.
